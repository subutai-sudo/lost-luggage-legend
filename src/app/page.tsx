...
function WaveDivider({ flip, to }: { flip?: boolean; to: string }) {
  return (
    <div className={`relative w-full overflow-hidden leading-[0] ${flip ? 'rotate-180' : ''}`} style={{ marginTop: '-1px' }}>
      <svg viewBox="0 0 1440 100" preserveAspectRatio="none" className="w-full h-[80px] md:h-[100px] wave-animate">
        <path d="M0,50 C240,100 480,0 720,50 C960,100 1200,0 1440,50 L1440,100 L0,100 Z" fill={to} />
      </svg>
    </div>
  )
}
...
function GoldOrnament({ label }: { label?: string }) {
  return (
    <div className="flex items-center justify-center gap-4 py-6">
      <span className="h-px w-20 bg-gradient-to-r from-transparent to-[#c9a96e]/40" />
      {label ? (
        <span className="text-[0.6rem] uppercase tracking-[0.3em] font-semibold" style={{ color: '#c9a96e' }}>{label}</span>
      ) : (
        <svg width="10" height="10" viewBox="0 0 10 10" fill="#c9a96e"><path d="M5 0 L10 5 L5 10 L0 5 Z" /></svg>
      )}
      <span className="h-px w-20 bg-gradient-to-l from-transparent to-[#c9a96e]/40" />
    </div>
  )
}
...
export default function Home() {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        document.querySelectorAll('.hero-blob').forEach((b, i) => {
          ;(b as HTMLElement).style.transform = `translateY(${y * (0.3 + i * 0.12)}px) scale(${1 + y * 0.0002})`
        })
        document.querySelectorAll('.parallax-slow').forEach((b) => {
          ;(b as HTMLElement).style.transform = `translateY(${y * 0.08}px)`
        })
      })
    }
  ...
  return (
    <>
      ...
      <section className="relative min-h-[90vh] overflow-hidden" style={{ backgroundColor: 'var(--color-bg)' }}>
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-12 py-16 lg:py-24">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="order-1 space-y-8 reveal-parent">
              ...
              <div className="space-y-4">
                <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-bold leading-[1.1] tracking-tight" style={{ color: 'var(--color-ink)' }}>
                  Travel Smarter.<br />
                  <span className="bg-gradient-to-r from-[#0891b2] via-[#0e7490] to-[#c9a96e] bg-clip-text text-transparent">Not Harder.</span>
                </h1>
                <p className="text-xl md:text-2xl leading-relaxed max-w-xl font-light" style={{ color: 'var(--color-ink-light)' }}>
                  Expert destination guides with price comparison tools from trusted travel suppliers. Your perfect trip starts with perfect planning.
                </p>
              </div>
              ...
              <div className="flex items-center gap-4 pt-6 border-t" style={{ borderColor: 'var(--color-border)' }}>
                <div className="flex -space-x-3">
                  {['photo-1494790108377-be9c29b29330', 'photo-1507003211169-0a1dd7228f2d', 'photo-1438761681033-6461ffad8d80', 'photo-1500648767791-00dcc994a43e'].map((id, i) => (
                    <img key={i} src={`https://images.unsplash.com/${id}?w=50&h=50&fit=crop`} alt="" className="w-10 h-10 rounded-full border-2 object-cover" style={{ borderColor: 'var(--color-bg)' }} />
                  ))}
                </div>
                <div>
                  <div className="flex items-center gap-1">{[1,2,3,4,5].map(i => <svg key={i} width="16" height="16" viewBox="0 0 24 24" fill="#c9a96e"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>)}</div>
                  <p className="text-sm" style={{ color: 'var(--color-muted)' }}><span className="font-semibold" style={{ color: 'var(--color-ink)' }}>4.9/5</span> from 2,400+ travelers</p>
                </div>
              </div>
            </div>
            <div className="order-2 relative reveal-parent">
              <div className="relative grid grid-cols-2 gap-4">
                <div className="col-span-2 rounded-2xl overflow-hidden shadow-2xl" style={{ aspectRatio: '16/10' }}>
                  <img src="https://images.unsplash.com/photo-1506929562872-bb421503ef21?w=1200&q=90" alt="Stunning tropical beach destination" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
                  <div className="absolute bottom-6 left-6 right-6">
                    <div className="backdrop-blur-md rounded-xl p-4 shadow-lg" style={{ backgroundColor: 'rgba(255,255,255,0.95)' }}>
                      <div className="flex items-center justify-between">
                        ...
                        <a href="/guides/maldives" className="px-4 py-2 rounded-lg text-sm font-semibold text-white" style={{ background: 'linear-gradient(135deg, #0891b2 0%, #0e7490 100%)' }}>View Guide</a>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="rounded-xl overflow-hidden shadow-lg" style={{ aspectRatio: '4/3' }}>
                  <img src="https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?w=600&q=90" alt="Tokyo cityscape" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
                </div>
                <div className="rounded-xl overflow-hidden shadow-lg relative" style={{ aspectRatio: '4/3' }}>
                  <img src="https://images.unsplash.com/photo-1533105079780-92b9be482077?w=600&q=90" alt="Santorini Greece" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
                  <div className="absolute inset-0 flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.3)' }}><span className="text-white font-bold text-lg">+97 more</span></div>
                </div>
              </div>
      ...
      {/* Transition: Hero → Compare Prices */}
      <div style={{ backgroundColor: 'var(--color-bg)' }}><GoldOrnament label="Compare Prices" /></div>

      {/* Compare Prices */}
      <div className="reveal"><ComparePricesSection /></div>
      <WaveDivider to="#f9f6f0" />

      {/* Goldilock Zone */}
      <div className="reveal"><GoldilockZone /></div>
      ...
      <section id="newsletter" className="py-20 lg:py-28 relative overflow-hidden reveal" style={{ backgroundColor: 'var(--color-bg-warm)' }}>
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-12">
          <div className="max-w-2xl mx-auto text-center mb-12">
            ...
            <h2 className="font-display text-4xl md:text-5xl font-bold tracking-tight mb-4" style={{ color: 'var(--color-ink)' }}>Never Miss a Great Deal</h2>
      ...
      <footer style={{ backgroundColor: 'var(--color-footer-bg)', borderTop: '1px solid var(--color-border)' }} className="pt-16 pb-8">
        <div className="max-w-7xl mx-auto px-6 lg:px-12">
          <div className="grid md:grid-cols-4 gap-10 mb-12">
            <div className="md:col-span-2">
              <div className="flex items-center gap-3 mb-5">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#0891b2] to-[#0e7490] flex items-center justify-center shadow-lg"><span className="text-white text-xl">🧳</span></div>
                <div>
              ...
              <p className="text-sm leading-relaxed max-w-sm mb-4" style={{ color: 'var(--color-muted)' }}>
                Independent travel journalism. No sponsorships, no free trips, no conflicts. We only earn affiliate commissions when you find a guide genuinely useful.
              </p>
            </div>
            <div>
              ...
              <ul className="space-y-2.5">
                {[
                  ...
                  <li key={item.label}>
                    <a href={item.href} target={item.external ? '_blank' : undefined} rel={item.external ? 'noopener noreferrer sponsored' : undefined} className="text-sm transition-colors hover:text-[#0891b2]" style={{ color: 'var(--color-muted)' }}>{item.label}</a>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="border-t pt-6 flex flex-col md:flex-row items-center justify-between gap-3" style={{ borderColor: 'var(--color-border)' }}>
            <p className="text-xs" style={{ color: 'var(--color-muted)' }}>&copy; {new Date().getFullYear()} Lost Luggage Legend. Affiliate links may earn commissions at no extra cost to you.</p>
            <p className="text-xs" style={{ color: 'var(--color-muted)' }}>Prices and availability change. Confirm directly with suppliers before booking.</p>
          </div>
        </div>
      </footer>
    </>
